...
//隐藏左边滚动条   
$(function () {
    $(".mainBlock .left").panel({ iWheelStep: 32 });
    //获取策略列表
    policFun.getPolicyList();
    // //分组部署策略列表
    setTimeout(function () {
        policFun.tacticsArrange();
    }, 100)
});
//策略开关样式
    ...
    this.getPolicyList = function () {
        hr.myAjax({
            ...
            for (i = 0; i < list.length; i++) {
                this.select += "<option policyid=" + list[i].policy_id + ">" + safeStr(list[i].policy_name) + "</option>";
            }
        });
    };
    this.selectAllFP = function () {
        hr.selectAll(checkbox, "table", '.selectFP');
    },
        this.tacticsArrange = function () {
            ...
            hr.myAjax({
                ...
                var data = data.data;
                var list = data.list;
                var ungrouped = data.ungrouped;
                var tableth = "";
                tableth += "<tr groupid=" + ungrouped.group_id + ">";
                tableth += "<td><img src='images/computer1.png'>未分组终端</td>";
                tableth += "<td>" + ungrouped.clients + "</td>";
                tableth += "<td>";
                ...
                $("tr[groupid=" + ungrouped.group_id + "] select option[policyid=" + ungrouped.policy_id + "]").attr("selected", true);
                ...
                for (i = 0; i < list.length; i++) {
                    ...
                    table += "<td><img src='images/computer11.png'><span>" + safeStr(list[i].group_name) + "</span></td>";
                    ...
                    $("tr[groupid=" + groupid + "] select option[policyid=" + grouppolicyid + "]").attr("selected", true);
                }
            });
        }
}
var policFun = new Polic();
...
$("input[type=checkbox]").change(function () {
    ...
    var textIpt = parent.find("input[type=text]");
    var placeholder = parent.find(".placeholder");
    var disabled = false;
    if (!this.is(":checked")) {
        disabled = true;
    }
    (textIpt.val() == '') ? placeholder.show() : placeholder.hide();
    textIpt.prop("disabled", disabled);
})
...
$(".systemRooting .tr").click(function () {
    ...
    $this.find(".deploypng").css({ background: "url(../images/"+bg+".png)", backgroundPosition: "-18px" })
})
//排序
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.errno == -1 && data.errmsg.indexOf("1062") > 0) {
                    ...
                    $(".bgContainer .table table").html("<tr class='th' id='tableAlign'><td width='60%'>终端分组</td><td width='20%'>包含终端数</td><td width='20%'>部署防护策略</td></tr>");
                    select = "";
                    policylist();
                    setTimeout(function () {
                        policFun.tacticsArrange();
