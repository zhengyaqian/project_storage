	...
	__creator:function(ele){
	  ...
	  //事件对象
		...
		oEvent[sMouseWheel] = function(ev){
			...
			// 如果是终端管理页面分组滚动时的事件
			if(parent.$("#mainFrame").attr("src")=="terminalManage.html"){
				if((Math.abs(($(".groupsPageContainer").height()-$(".groupsPage").height()))==Math.abs($(".groupsPageContainer").position().top))){
					$(".moreGroupsButton a").css('background', 'url(images/lessgroup.png)');
				}else{
					$(".moreGroupsButton a").css('background', 'url(images/moregroup.png)');
				}
			}
